...
main() {
    ...
    aws ${_profile} --region "${_region}" s3 cp --acl public-read --recursive --exclude "*" --include "*substack.cfn.json" --include "*substack.cfn.yaml" ${_srcdir}/cloudformation/ s3://${_templates_folder}/ || _error_exit 'Failed to push substack cfn templates to S3'
