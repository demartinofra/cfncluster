...
def _terminate_compute_nodes(event):
    try:
        ...
        logger.error("Failed when terminating compute nodes with error %s", e)
        raise


def _describe_instance_ids_iterator(stack_name):
    ec2 = boto3.client("ec2", config=boto3_config)
    filters = [
...
ACTION_HANDLERS = {
    ...
    "TERMINATE_EC2_INSTANCES": _terminate_compute_nodes,
}
