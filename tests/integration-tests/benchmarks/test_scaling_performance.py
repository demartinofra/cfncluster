...
METRIC_WIDGET_TEMPLATE = """
    {{
        "metrics": [
            [ "ParallelCluster/benchmarking/{cluster_name}", "ComputeNodesCount", {{ "stat": "Maximum", "label": "ComputeNodesCount Max" }} ],
            [ "...", {{ "stat": "Minimum", "label": "ComputeNodesCount Min" }} ],
            [ "AWS/AutoScaling", "GroupDesiredCapacity", "AutoScalingGroupName", "{asg_name}", {{ "stat": "Maximum", "label": "GroupDesiredCapacity" }} ],
...
def test_scaling_performance(region, scheduler, os, instance, pcluster_config_reader, clusters_factory, request):
    ...
    cluster_config = pcluster_config_reader(scaledown_idletime=benchmark_params["scaledown_idletime"])
